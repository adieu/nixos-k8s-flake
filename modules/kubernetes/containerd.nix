{ pkgs, lib, config, ... }:
let
  cfg = config.services.k8s.cri.containerd;
...
in
{
  ...
  options.services.k8s.cri.containerd = with lib.types; {
  ...
  config = lib.mkIf cfg.enable {
    warnings = lib.optional (cfg.configFile != null) ''
      `services.k8s.cri.containerd.configFile` is deprecated. use `services.k8s.cri.containerd.settings` instead.
    '';
